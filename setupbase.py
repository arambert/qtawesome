...
class UpdateFA6Command(setuptools.Command):
    ...
    FA_STYLES = ('regular', 'solid', 'brands', 'light', 'duotone', 'thin')
    ...
    TTF_PATH_TEMPLATE = os.path.join(
        ...
        "qtawesome",
        "fonts",
        "fontawesome6-{style}-webfont-{version}.ttf"
    )
    ...
    def run(self):
        ...
        for style, details in icons.items():
            ...
            if style in ('regular', 'solid', 'light', 'duotone', 'thin'):
                new_name = str("Font Awesome 6 %s") % style.title()
